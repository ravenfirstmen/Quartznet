...
namespace Quartz.Web.History
{
    public class JobHistoryDelegate
    {
            ...
            get
            {
                lock (this)
                {
                    if (driverDelegate == null)
                        try
                        {
                            if (delegateTypeName != null) delegateType = TypeLoadHelper.LoadType(delegateTypeName);
                            ...
                            var ctor = delegateType.GetConstructor(new Type[0]);
                            if (ctor == null) throw new InvalidConfigurationException("Configured delegate does not have public constructor that takes no arguments");
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            var entries = new List<JobHistoryEntryDto>();
            using (var dbConnection = GetConnection(IsolationLevel.ReadUncommitted))
            {
                using (var dbCommand = Delegate.PrepareCommand(dbConnection, sql))
                {
                    ...
                    using (var reader = await dbCommand.ExecuteReaderAsync(cancellationToken).ConfigureAwait(false))
                    {
                        ...
                        {
                            var entry = new JobHistoryEntryDto
                            {
        ...
        protected virtual ConnectionAndTransactionHolder GetConnection(IsolationLevel isolationLevel)
        {
            ...
            if (conn == null) throw new JobPersistenceException($"Could not get connection from DataSource '{dataSource}'");

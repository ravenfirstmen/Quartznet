...
namespace Quartz.AspNetCore.Server.Dtos
{
    public class CalendarDetailDto
    {
        protected CalendarDetailDto(ICalendar calendar)
        {
            ...
            if (calendar.CalendarBase != null) CalendarBase = Create(calendar.CalendarBase);
        }
        ...
        public static CalendarDetailDto Create(ICalendar calendar)
        {
            ...
            if (annualCalendar != null) return new AnnualCalendarDto(annualCalendar);

            var cronCalendar = calendar as CronCalendar;
            if (cronCalendar != null) return new CronCalendarDto(cronCalendar);

            var dailyCalendar = calendar as DailyCalendar;
            if (dailyCalendar != null) return new DailyCalendarDto(dailyCalendar);

            var holidayCalendar = calendar as HolidayCalendar;
            if (holidayCalendar != null) return new HolidayCalendarDto(holidayCalendar);

            var monthlyCalendar = calendar as MonthlyCalendar;
            if (monthlyCalendar != null) return new MonthlyCalendarDto(monthlyCalendar);

            var weeklyCalendar = calendar as WeeklyCalendar;
            if (weeklyCalendar != null) return new WeeklyCalendarDto(weeklyCalendar);
